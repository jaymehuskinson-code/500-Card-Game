...
export function LobbyPage({ onJoinGame }: LobbyPageProps) {
  ...
  useGameRealtime(pendingGameId);

  // Subscribe to lobby updates when in a pending game
  useEffect(() => {
  ...
  const handleCreate = async () => {
    ...
    try {
      const { data: game, error: e } = await supabase.from('games')
        .insert({ host_id: user.id, phase: 'lobby' })
        .select().single();
      if (e) throw e;
  ...
  const handleStartGame = async () => {
    ...
    setLoading(true); setError('');
    try {
      await callEdgeFunction('deal-cards', { game_id: pendingGameId });
            ...
            <div className="bg-table-dark border border-gold/30 rounded-2xl shadow-2xl p-8 text-center">
              ...
              <div className="space-y-3">
                <button onClick={handleCreate} disabled={loading}
                  className="w-full bg-gold text-black font-display font-bold py-4 rounded-xl text-lg
                    hover:bg-gold/90 transition shadow-lg shadow-gold/20 tracking-wide">
                  Create New Game
                </button>
                <button onClick={() => setView('join')}
                  className="w-full bg-white/5 text-white border border-white/15 font-body py-4 rounded-xl text-lg
              ...
              <button onClick={() => supabase.auth.signOut()}
                className="mt-6 text-gray-600 hover:text-gray-400 text-sm transition font-body">
                Sign out
              </button>
            </div>
          </motion.div>
        ...
        {view === 'join' && (
          ...
          >
            <div className="bg-table-dark border border-gold/30 rounded-2xl shadow-2xl p-8">
              <button onClick={() => setView('main')} className="text-gray-500 hover:text-gray-300 mb-6 flex items-center gap-1 text-sm transition font-body">
                ← Back
              </button>
              ...
              <input
                ...
                className="w-full bg-black/30 border border-white/10 text-white rounded-lg px-4 py-3
                  focus:outline-none focus:border-gold/60 text-center text-2xl font-display tracking-widest
                  placeholder-gray-700 mb-4"
              />
              {error && <p className="text-red-400 text-sm mb-3">{error}</p>}
              <button onClick={handleJoin} disabled={loading || roomCode.length < 4}
                className="w-full bg-gold text-black font-display font-bold py-3 rounded-lg text-lg
                  hover:bg-gold/90 disabled:opacity-40 transition">
            ...
            <div className="bg-table-dark border border-gold/30 rounded-2xl shadow-2xl p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-center">
                  <div className="text-3xl font-display text-white tracking-widest bg-black/40 px-4 py-2 rounded-lg border border-white/10">
                  ...
                  return (
                    <div key={seat} className={`absolute ${positions[seat]}`}>
                      <div className={`text-center px-3 py-2 rounded-lg ${player ? 'bg-black/60 border border-white/20' : 'border border-dashed border-white/10'}`}>
                        {player ? (
                          <>
                            <div className={`text-xs font-display ${teamColors[seat]}`}>
              ...
              {isHost ? (
                <button onClick={handleStartGame} disabled={!canStart || loading}
                  className="w-full bg-gold text-black font-display font-bold py-4 rounded-xl text-xl
                    hover:bg-gold/90 disabled:opacity-30 transition shadow-lg shadow-gold/20 tracking-wide">
                  {loading ? 'Starting...' : 'Start Game'}
                </button>
              ) : (
                <div className="text-center text-gray-500 font-body py-3 animate-pulse">
