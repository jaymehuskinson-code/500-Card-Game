...
export function LobbyPage({ onJoinGame }: LobbyPageProps) {
  ...
  // ── Realtime updates for waiting room ────────────────────
  useEffect(() => {
    if (!pendingGameId) return;
    const channel = supabase.channel(`lobby:${pendingGameId}`)
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'games', filter: `id=eq.${pendingGameId}`
      }, async () => {
      ...
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'game_players', filter: `game_id=eq.${pendingGameId}`
      }, async () => {
        const { data: ps } = await supabase
          .from('game_players').select('*, profiles(*)').eq('game_id', pendingGameId);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const { data: ps } = await supabase.from('game_players')
        .select('*, profiles(*)').eq('game_id', game.id);
      setPendingGame(game);
      setPendingPlayers(ps ?? []);
          ...
          <motion.div key="create" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }}
            ...
            <div className="bg-table-dark border border-gold/30 rounded-2xl shadow-2xl p-8">
              <button onClick={() => setView('main')}
                className="text-gray-500 hover:text-gray-300 mb-6 flex items-center gap-1 text-sm transition font-body">
                ← Back
              </button>
              ...
              <input type="text" value={gameName}
                ...
                placeholder="e.g. Friday Night 500"
                maxLength={30}
                      ...
                      <div key={game.id}
                        ...
                        <div>
                          ...
                          <p className="text-gray-500 text-xs font-body mt-0.5">
                            {playerCount}/4 players · Code: {game.room_code}
                          </p>
                        </div>
                        <button
                          onClick={() => !isFull && handleJoin(game.id)}
                          disabled={isFull || loading}
                          className={`px-4 py-2 rounded-lg font-display text-sm transition ${
                            isFull
                  ...
                  return (
                    <div key={seat} className={`absolute ${seatPositions[seat]}`}>
                      <motion.div
                        ...
                        className={`text-center px-3 py-2 rounded-lg min-w-[90px] ${
                          player ? 'bg-black/60 border border-white/20' : 'border border-dashed border-white/10'
                        }`}>
                        {player ? (
                          <>
                            ...
                            {player.player_id === user?.id && (
                              <div className="text-gold text-xs">(you)</div>
                            )}
                          </>
                        ) : (
                          <div className="text-gray-600 text-xs font-body px-2 py-1">
                            Waiting...
                          </div>
                        )}
                      </motion.div>
                ...
                <div className="flex gap-1">
                  {[0,1,2,3].map(i => (
                    <div key={i} className={`w-3 h-3 rounded-full transition-all duration-300 ${
                      i < pendingPlayers.length ? 'bg-gold' : 'bg-gray-700'
                    }`} />
                  ))}
                  <span className="text-gray-400 font-body text-sm ml-2">
                    {pendingPlayers.length}/4 players
                  </span>
                </div>
