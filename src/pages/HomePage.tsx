...
type Modal = null | 'auth' | 'create' | 'waiting';
...
export function HomePage({ onJoinGame }: HomePageProps) {
  ...
  const handleAuth = async () => {
    setError('');
    if (!emailInput.trim() || !passwordInput.trim()) { setError('Email and password required'); return; }
    if (authMode === 'signup' && nameInput.trim().length < 2) { setError('Name must be at least 2 characters'); return; }
    setLoading(true);
  ...
  const handleLeaveWaiting = async () => {
    ...
    if (pendingGame?.host_id === user.id) await supabase.from('games').delete().eq('id', pendingGameId);
    setPendingGameId(null); setPendingGame(null); setPendingPlayers([]);
    setModal(null); setLoading(false); fetchGames();
  };
  ...
  return (
    <div className="min-h-screen bg-felt font-body">
      ...
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between px-8 pt-8 pb-4 border-b border-white/5">
        <div>
          ...
          {profile ? (
            <>
              <div className="text-right">
                ...
                <p className="text-gray-600 text-xs">{profile.games_played ?? 0} games played</p>
              </div>
              <button onClick={handleSignOut}
                className="text-gray-500 hover:text-gray-300 text-sm transition border border-white/10 px-3 py-1.5 rounded-lg hover:border-white/20">
                Sign out
              </button>
            </>
          ) : (
            <div className="flex gap-2">
              <button onClick={() => { setAuthMode('signin'); setModal('auth'); setError(''); setPendingAction(null); }}
                className="text-gray-400 hover:text-white text-sm transition border border-white/10 px-4 py-2 rounded-lg hover:border-white/20">
                Sign in
              </button>
              <button onClick={() => { setAuthMode('signup'); setModal('auth'); setError(''); setPendingAction(null); }}
                className="text-black bg-gold hover:bg-gold/90 text-sm font-bold transition px-4 py-2 rounded-lg font-display">
                Create account
              </button>
            </div>
          )}
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-8 py-8">
        ...
        {error && !modal && (
          <div className="bg-red-900/30 border border-red-500/30 rounded-xl px-4 py-3 mb-4 text-red-400 text-sm">{error}</div>
        )}

        {openGames.length === 0 ? (
          <div className="text-center py-24 text-gray-600">
            <p className="text-5xl mb-4">🃏</p>
            <p className="text-lg font-display text-gray-500">No open games right now</p>
            <p className="text-sm mt-2">Create one and invite your friends</p>
          </div>
        ) : (
            ...
            {openGames.map(game => {
              ...
              return (
                <motion.div key={game.id} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
                  ...
                  <button onClick={() => handleJoinClick(game.id)} disabled={isFull || loading}
                    className={`px-6 py-2.5 rounded-xl font-display text-sm font-bold transition ${
        ...
        <button onClick={fetchGames} className="mt-4 text-gray-700 hover:text-gray-500 text-xs transition w-full text-center">↻ Refresh</button>
      </div>
            ...
            <motion.div initial={{ scale:0.9, y:20 }} animate={{ scale:1, y:0 }} exit={{ scale:0.9 }}
              ...
              <div className="flex bg-black/30 rounded-xl p-1 mb-6">
                <button onClick={() => { setAuthMode('signup'); setError(''); }}
                  className={`flex-1 py-2 rounded-lg text-sm font-display transition ${authMode === 'signup' ? 'bg-gold text-black font-bold' : 'text-gray-400 hover:text-white'}`}>
                  New player
                </button>
                <button onClick={() => { setAuthMode('signin'); setError(''); }}
                  className={`flex-1 py-2 rounded-lg text-sm font-display transition ${authMode === 'signin' ? 'bg-gold text-black font-bold' : 'text-gray-400 hover:text-white'}`}>
                  Sign in
                </button>
              </div>
              ...
              <div className="space-y-3">
                {authMode === 'signup' && (
                  <div>
                    <label className="block text-gray-400 text-xs mb-1 font-body">Display name</label>
                    <input type="text" value={nameInput} onChange={e => setNameInput(e.target.value)}
                      placeholder="Your name at the table" maxLength={20} autoFocus
                      className="w-full bg-black/30 border border-white/10 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-gold/60 placeholder-gray-600 font-body" />
                  </div>
                )}
                <div>
                  <label className="block text-gray-400 text-xs mb-1 font-body">Email</label>
                  <input type="email" value={emailInput} onChange={e => setEmailInput(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && handleAuth()}
                    placeholder="you@example.com" autoFocus={authMode === 'signin'}
                    className="w-full bg-black/30 border border-white/10 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-gold/60 placeholder-gray-600 font-body" />
                </div>
                <div>
                  <label className="block text-gray-400 text-xs mb-1 font-body">Password</label>
              ...
              <p className="text-center text-gray-700 text-xs mt-4 font-body">
                {authMode === 'signup' ? 'Already have an account? ' : 'New player? '}
                <button onClick={() => { setAuthMode(authMode === 'signup' ? 'signin' : 'signup'); setError(''); }}
            ...
            <motion.div initial={{ scale:0.9, y:20 }} animate={{ scale:1, y:0 }} exit={{ scale:0.9 }}
              ...
              <div className="flex gap-3">
                <button onClick={() => { setModal(null); setError(''); }}
                  className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400 hover:text-white transition font-body">Cancel</button>
            ...
            <motion.div initial={{ scale:0.9, y:20 }} animate={{ scale:1, y:0 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-400 text-sm ml-1">{pendingPlayers.length}/4</span>
                </div>
              ...
              {isHost ? (
                ...
                <motion.div animate={{ opacity:[0.5,1,0.5] }} transition={{ repeat:Infinity, duration:2 }}
                  className="text-center text-gray-500 py-3">Waiting for host to start...</motion.div>
              )}
